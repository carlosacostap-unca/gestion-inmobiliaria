...
import NuevoClienteModal from "@/app/loteos/[loteoId]/NuevoClienteModal";
import { createSupabaseBrowserClient } from "@/lib/supabase-browser";

type Cliente = { id: string; nombre_completo: string; telefono?: string };

export default function AlquilarModal({ onSubmit }: { onSubmit: (formData: FormData) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const buscar = async () => {
      ...
      const { data } = await supabase
        ...
        .limit(10);
      if (!cancelado) setResultados((data as Cliente[]) || []);
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-3xl rounded-lg border border-zinc-800 bg-zinc-950 p-5 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Registrar alquiler</h3>
            ...
            <form action={onSubmit} className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="sm:col-span-3 flex items-center gap-2 relative">
                <input value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar cliente..." className="flex-1 rounded-md bg-zinc-900 text-zinc-100 px-3 py-2 ring-1 ring-zinc-800" />
                <NuevoClienteModal onCreated={(c) => { setSeleccionado(c); setBusqueda(c.nombre_completo); setResultados([]); }} />
                {busqueda && resultados.length > 0 && (
                  <ul className="absolute top-full left-0 right-0 mt-1 max-h-56 overflow-auto rounded-md border border-zinc-800 bg-zinc-950 shadow-lg z-10">
                    {resultados.map((c) => (
                      <li key={c.id}>
                        <button type="button" className="w-full text-left px-3 py-2 hover:bg-zinc-800" onClick={() => { setSeleccionado(c); setBusqueda(c.nombre_completo); setResultados([]); }}>
              ...
              <input name="precio_mensual" placeholder="Precio mensual" type="number" step="0.01" className="rounded-md bg-zinc-900 text-zinc-100 px-3 py-2 ring-1 ring-zinc-800" required />
              <input name="deposito" placeholder="Depósito" type="number" step="0.01" className="rounded-md bg-zinc-900 text-zinc-100 px-3 py-2 ring-1 ring-zinc-800" />
              <input name="fecha_inicio" placeholder="Fecha inicio" type="date" className="rounded-md bg-zinc-900 text-zinc-100 px-3 py-2 ring-1 ring-zinc-800" required />
              <input name="fecha_fin" placeholder="Fecha fin (opcional)" type="date" className="rounded-md bg-zinc-900 text-zinc-100 px-3 py-2 ring-1 ring-zinc-800" />
              ...
              <div className="sm:col-span-3 mt-2 flex justify-end gap-2">
                ...
                <button disabled={!seleccionado} type="submit" className="rounded-md bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white px-4 py-2 font-medium">Confirmar alquiler</button>
              </div>
            </form>
          </div>
        </div>
